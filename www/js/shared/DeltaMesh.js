...
(function(exports) {
    ...
    DeltaMesh.prototype.refineRed = function(tetra) {
        ...
        var midpts = [
            t[3].interpolate(t[0], 0.5), // tip midpoint
            t[3].interpolate(t[1], 0.5), // tip midpoint
            t[3].interpolate(t[2], 0.5), // tip midpoint
            t[0].interpolate(t[1], 0.5), // base midpoint
            t[1].interpolate(t[2], 0.5), // base midpoint
            t[2].interpolate(t[0], 0.5), // base midpoint
        ];
        ...
        return tetra.partitions = [
            ...
            new Tetrahedron(midpts[0], midpts[1], midpts[2], midpts[5], tetra),
            new Tetrahedron(midpts[3], midpts[4], midpts[5], midpts[1], tetra),
            new Tetrahedron(midpts[3], midpts[5], midpts[0], midpts[1], tetra),
            new Tetrahedron(midpts[4], midpts[5], midpts[1], midpts[2], tetra),
        ];
    }
    DeltaMesh.prototype.tetrahedron = function(xyz, level) {
        ...
        if (that.root.contains(xyz)) {
            var zfloor = that.zfloor(xyz.z, level);
        ...
        that.logger.debug("DeltaMesh.tetrahedron(", xyz, ") probes:", probes);
        return tetra;
    }
...
(typeof describe === 'function') && describe("DeltaMesh", function() {
    ...
    it("refineZ(z,level) should refine the mesh at the given Z to the given level", function() {
        ...
        //var lvl1l = mesh.refineZ(-40, 1);
        //lvl1l.length.should.equal(7);
    ...
    it("tetrahedron(xyz) should return tetrahedron containing xyz", function() {
        var mesh = new DeltaMesh(options);
        var tetra = mesh.tetrahedron({x:50, y:5, z:-40});
        var e=0.01;
        tetra.t[0].equal({x:54.13,y:6.25,z:-50},e).should.True;
        tetra.t[1].equal({x:54.13,y:18.75,z:-50},e).should.True;
        tetra.t[2].equal({x:43.3,y:12.5,z:-37.5},e).should.True;
        tetra.t[3].equal({x:54.13,y:-6.25,z:-37.5},e).should.True;
    })
})
