...
(function(exports) {
    ////////////////// constructor
    function FpdPlanner(model, mto, driver, options) {
        var that = this;
        ...
        Logger.start("FpdPlanner() driver:" + driver.name);

        return that;
    }

    FpdPlanner.prototype.beforeRebase = function() {
        var that = this;
        if (that.serialPath !== that.model.rest.serialPath) {
            Logger.start('FpdPlanner: new serial path:', that.model.rest.serialPath);
    ...
    FpdPlanner.prototype.syncModel = function(data) {
        var that = this;
        if (data) {
            ...
            //console.log("FpdPlanner.syncModel() data:" + JSON.stringify(data));
            ...
            if (serialPath !== that.model.rest.serialPath) {
                console.log('INFO\t: FpdPlanner: new serial path:', that.model.rest.serialPath);
                if (that.model.available) {
                    that.driver.close();
                    setTimeout(function() {
                        that.driver.open(function() {
    ...
    FpdPlanner.prototype.moveLPP = function(x, y, z, onDone) {
        var that = this;
    ...
    FpdPlanner.prototype.isLPPMove = function(cmd) {
        var that = this;
        ...
        if (that.model.rest.lppSpeed <= 0) {
            //console.log("FpdPlanner.isLPPMove(lppSpeed <= 0) => false");
            return false;
        }
        if (!cmd.mov.hasOwnProperty("x") ||
            !cmd.mov.hasOwnProperty("y") ||
            !cmd.mov.hasOwnProperty("z")) {
            //console.log("FpdPlanner.isLPPMove(not absolute) => false");
            return false;
        }
        if (cmd.mov.lpp === false) {
            //console.log("FpdPlanner.isLPPMove(lpp:false) => false");
            return false;
        }
        return true;
    }
    FpdPlanner.prototype.mpoPlanSetXYZ = function(x, y, z, options) {
        var that = this;
        options = options || {};
    ...
    FpdPlanner.prototype.mpoPlanSetPulses = function(p1, p2, p3, options) {
        var that = this;
        options = options || {};
        var pulses = {
            p1: p1,
    ...
    FpdPlanner.prototype.send1 = function(cmd, onDone) {
        var that = this;
        ...
        that.verbose && mpoPlan && console.log("DEBUG\t: FpdPlanner.send1 mpoPlan:" + JSON.stringify(mpoPlan));
        if (sendCmd) {
            that.driver.pushQueue(cmd, onDone);
        }
        that.driver.processQueue();
    }
    FpdPlanner.prototype.send = function(jobj, onDone) {
        var that = this;

        if (!onDone) {
            onDone = function(data) {
                if (data.s) {
                    console.log("TTY \t: FpdPlanner: FireStep response:" + data.s);
                }
            }
        }
        ...
        if (that.driver.queueLength()) {
            console.log("TTY \t: FpdPlanner.send() queued:", that.driver.queueLength());
        }
        return that;
    }
    FpdPlanner.prototype.cmd_mpo = function() {
        var that = this;
        return {
            mpo: "",
            //dpyds: 12,
    ...
    FpdPlanner.prototype.onSerialResponse = function(response) {
        var that = this;
        var r = response.r;
        //console.log("DEBUG\t: FpdPlanner.onSerialResponse(" + JSON.stringify(response) + ")");
    ...
    FpdPlanner.prototype.onIdle = function() {
        var that = this;
        if (that.model.response && that.model.response.mpo) {
            ...
            that.mpoPlanSetPulses(mpo["1"], mpo["2"], mpo["3"], {
                log: "FpdPlanner.onIdle(initialized)"
            });
        } else {
            that.verbose && console.log("TTY \t: FpdPlanner.onIdle(waiting) ...");
        }
        if (that.mpoPlan) {
    ...
    FpdPlanner.prototype.onStartup = function(err) {
        ...
        var that = this;
        if (err == null) {
            //console.log("INFO\t: FpdPlanner.onStartup() available:", that.available);
    ...
    module.exports = exports.FpdPlanner = FpdPlanner;
...
(typeof describe === 'function') && describe("planner", function() {
    ...
    it("planner works with MockCartesian", function() {
        ...
        console.log("TESTIT");
        var planner = new FpdPlanner(model, mto, driver, options);
